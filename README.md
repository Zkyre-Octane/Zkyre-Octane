````md
<div align="center"> ```text ⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢀⡀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀ ⠀⠀⠀⠀⠀⣏⠉⠩⠥⢄⡔⠉⢸⣦⠤⠄⢀⣀⠀⠀⠀⠀⠀⠀⠀ ⠀⠀⠀⠀⠀⢹⢰⠀⢰⡟⠀⠰⠘⣟⣳⡉⠓⠒⠪⠴⣂⠄⡀⠀⠀ ⠀⠀⠀⠀⠀⠈⡄⡆⣦⣿⣾⡻⣿⣾⡿⣃⠀⠀⠀⠀⠀⠈⠉⠲⠄ Mauro Rocha ⠀⠀⠀⠀⠀⠀⠃⢀⡗⣿⡯⡛⢫⣽⣿⢿⠀⠀⠀⠀⠀⠀⠀⠀⠀ ------------------------- ⠀⠀⠀⠀⠀⠀⠸⠈⣷⣿⣿⣭⣤⣿⣃⣾⠀⠀⠀⠀⠀⠀⠀⠀⠀ OS : Linux ⠀⠀⠀⠀⢠⠔⢘⣆⢻⣳⡻⣿⣿⡧⠉⠉⡸⠒⣢⡀⠀⠀⠀⠀⠀ Host : Security Lab ⠀⠀⠀⣠⠿⠠⣼⣿⠜⣿⡚⢽⣿⣡⡪⣸⣣⠀⠈⢻⠀⠀⠀⠀⠀ Kernel : Offensive Security ⠀⠀⡔⣇⣤⣶⡟⠙⣧⢫⢚⣲⣿⣿⣗⢊⣤⡶⡄⢈⠇⠀⠀⠀⠀ Shell : bash / zsh ⠀⢸⠰⢈⣿⣦⡔⣾⣿⣿⠏⢠⣽⠟⡡⠂⢹⣷⠇⢸⠇⠀⠀⠀⠀ Focus : Penetration Testing ⢠⢃⣥⣼⠋⡟⡉⠘⣿⡏⡿⠿⣿⣍⠠⣀⢹⢻⠂⡏⠰⠀⠀⠀⠀ Status : Learning & Building ⢸⠊⢉⠌⢸⠱⡴⣷⣾⣿⢰⣾⣀⣀⠨⢋⣿⠀⢴⡇⡄⠀⠀⠀⠀ ⠈⠣⣻⠶⠷⠀⢠⠈⣇⢸⡚⣋⣩⠐⠁⣿⣩⣊⠌⢁⡇⠀⠀⠀⠀ ⠀⠀⠀⠉⠉⣷⣶⣇⣿⣿⢧⢹⠫⡄⣠⣿⣟⠓⠋⡌⠇⠀⠀⠀⠀ ⠀⠀⠀⠀⠀⢹⣹⣿⢿⣿⣿⡜⣽⡟⢸⠟⢠⣴⢢⣿⠀⠀
````

# `~/mauro`

> Penetration Testing & Offensive Security

```text
[+] Currently learning
    ├── Penetration testing
    ├── Vulnerability assessment
    ├── Exploitation techniques
    ├── Network & system security
    └── Linux / Windows security

[+] Training
    ├── TryHackMe
    ├── Cisco Networking Academy
    ├── CTFs
    └── Self-directed labs
```

## `fastfetch --security`

```text
╭────────────────────────────────────────────────────────────╮
│                    SECURITY TOOLKIT                        │
├────────────────────────────────────────────────────────────┤
│  Nmap              Network reconnaissance                  │
│  Burp Suite        Web application testing                 │
│  Metasploit        Exploitation framework                  │
│  Wireshark         Network analysis                        │
│  Hydra             Authentication testing                  │
│  John              Password auditing                        │
╰────────────────────────────────────────────────────────────╯
```

## `~/projects`

```text
RavenOS-Core
└── Core components of the RavenOS ecosystem

RavenOS-Umbra
└── Experimental security / offensive tooling

RavenOS-Whisper
└── Experimental communication / security project
```

## `whoami`

```bash
$ whoami
mauro

$ cat /etc/motd

Security student.
Linux enthusiast.
Always learning.
Always building.

$ echo $CURRENT_STATUS
[+] Studying
[+] Building
[+] Breaking things
[+] Learning how to fix them
```

## `connect`

```text
LinkedIn → www.linkedin.com/in/mauro-ponce369
```

---

```text
                    ┌─────────────────────────┐
                    │       SYSTEM READY      │
                    │                         │
                    │   Learn. Build. Break.  │
                    │        Repeat.          │
                    └─────────────────────────┘
```


